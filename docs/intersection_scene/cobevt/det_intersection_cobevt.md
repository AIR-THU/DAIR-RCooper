# README for Intersection_Cobevt
...
## Inference
We provide the inference script in codes/scripts/det_inference_intersection_cobevt.sh.
...
2. Run the command
    ...
    ```bash
    cd codes
    bash scripts/det_inference_intersection_cobevt.sh
    ```

## Training
We provide the training script in codes/scripts/det_train_intersection_cobevt.sh

To train a new model, do the following steps:

1. Setup the model parameters in a YAML file. We provide an example in configs/intersection_cobevt.yaml

2. Make sure the 'MODEL_CONFIG_DIR' in codes/scripts/det_train_intersection_cobevt.sh is right, and run the command

    ```bash
    cd codes
    bash scripts/det_train_intersection_cobevt.sh
    ```
